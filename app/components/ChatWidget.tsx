...
const QUICK_PROMPTS = [
  "What kind of work does Dr. Memari do?",
  "Tell me about NVIDIA workshops",
  "How do I apply for the AI Engineer Intern role?",
];
...
function formatChatConfigMessage(error: string) {
  if (error.includes("missing OPENAI_API_KEY")) {
    return "Chat isn’t configured: add OPENAI_API_KEY to your environment (e.g. Vercel → Project → Settings → Environment Variables), save, then redeploy. You can still reach Dr. Memari via the contact form below.";
  }
  return error;
}
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: leadName,
          email: leadEmail,
          message: `[Chat lead] ${leadNote || "Interested in follow-up from Majid Memari's site chat."}`,
          source: "chat-widget",
        }),
      });
      if (res.ok) {
          ...
          <div className="flex h-full w-full min-w-0 max-w-[100vw] flex-col rounded-none border-0 border-zinc-200 bg-white shadow-2xl sm:h-[min(560px,85vh)] sm:max-w-md sm:rounded-2xl sm:border sm:border-zinc-200 dark:border-zinc-800 dark:bg-zinc-950">
            <div className="flex min-w-0 items-center justify-between gap-2 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
              <div className="min-w-0">
                ...
                <p className="text-xs text-zinc-600 dark:text-zinc-500">Ask about Dr. Memari&apos;s work, workshops, or mentorship</p>
              </div>
              ...
              {messages.length === 0 && (
                <div className="space-y-3">
                  <p className="text-sm text-zinc-600 dark:text-zinc-500">
                    Hi — I can answer questions about Dr. Memari&apos;s work, NVIDIA workshops, public-sector
                    AI projects, and the AI Engineer Intern role. How can I help?
                  </p>
